...
class YouTubeDownloader:
    ...
    def create_widgets(self):
        ...
        #resize
        logo_image = logo_image.subsample(2,2)  # Reducir tamaño a la mitad
        logo_label = tk.Label(main_frame, image=logo_image, bg='white')
        logo_label.image = logo_image  # Guardar una referencia para evitar que se elimine
        logo_label.pack(pady=(0, 0))


        # Mensaje de bienvenida
        welcome_label = tk.Label(main_frame, text="¡Importante! - Usando este servicio financias la campaña de Sanchez para las generales de 2027", font=('Arial', 10), fg='orange',bg='white')
        ...
        self.url_entry = tk.Entry(
            ...
            bd=0,
            bg='white',
            fg='#999999')  # Color gris para placeholder
        ...
        second_line.pack(fill=tk.X, pady=(0, 10))  # Reducido de 10 a 8
        ...
        tk.Label(quality_frame, text="Calidad:", font=('Arial', 15), bg='white').pack(side=tk.LEFT)
        ...
        tk.Label(dir_frame, text="Guardar en:", font=('Arial', 15), bg='white').pack(side=tk.LEFT)
        
        self.dir_button = tk.Button(
            dir_frame,
            text=self.get_short_path(self.download_path, 20),
    ...
    def on_url_focus_in(self, event):
        ...
        if self.url_entry.get() == self.placeholder_text:
            ...
            self.url_entry.config(fg='black')  # Cambiar a texto negro
    
    def on_url_focus_out(self, event):
        """Maneja cuando el usuario sale del campo URL"""
        if not self.url_entry.get().strip():  # Si está vacío
            self.url_entry.insert(0, self.placeholder_text)
            self.url_entry.config(fg='#999999')  # Volver a gris
    ...
    def start_download(self):
        ...
        # Verificar si está vacío o contiene el placeholder
        if not url or url == self.placeholder_text:
            messagebox.showerror("Error", "Por favor bro, no va a funcionar si no pegas nada")
            return
            
        if not self.is_valid_youtube_url(url):
            messagebox.showerror("Error", "La URL no parece ser de YouTube. ¿Te crees que soy tonto?. Hasta Feijoó se daría cuenta...")
            return
    ...
    def download_success(self, video_title):
        ...
        self.url_entry.delete(0, tk.END)
        # Restaurar el placeholder después de limpiar
        self.url_entry.insert(0, self.placeholder_text)
    ...
    def download_error(self, error_msg):
        ...
        if "Video unavailable" in error_msg:
            user_msg = "El video no está disponible. Puede estar privado o haber sido eliminado. A ver que cosas me estas viendo..."
        elif "network" in error_msg.lower():
            user_msg = "Error de conexión. ¡Paga el internet!"
        else:
            user_msg = "Ha ocurrido un error durante la descarga, seguramente por otro apagón..."
